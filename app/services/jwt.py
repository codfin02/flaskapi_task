...
class JWTService:
    ...
    def _encode(self, data: dict[str, Any], expires_in: int) -> str:
        """
        파라미터로 전달받은 data를 Payload로 하고 유효기간이 expires_in 인 토큰을 인코딩하는 함수
        """
        payload = data.copy()
    ...
    def _decode(self, token: str) -> Any:
        """
        파라미터로 전달받은 토큰 값을 디코딩하여 페이로드를 반환하는 함수
        """
        try:
    ...
    def attach_jwt_token_in_response_cookie(
        ...
        """
        액세스 토큰, 리프레쉬 토큰, Response 객체를 넘겨받아 Response 객체의 쿠키에 액세스토큰과 리프레쉬 토큰을 추가하는 함수
        """
        response.set_cookie(
            key="access_token",
            value=access_token,
